...
/**
 * NOTE: this is a fork for `react-native-auto-scrolling`.
 * But it's modified because the lib throw type error while typescript is building
 */
export const AutoScrolling = ({
  ...
  function measureContainerView(event: LayoutChangeEvent) {
    const newContainerSize = isVertical ? event.nativeEvent.layout.height : event.nativeEvent.layout.width;
    if (isVertical ? containerHeight.current === newContainerSize : containerWidth.current === newContainerSize) {
      return;
    }
  ...
  function checkContent(newContentSize: number, fx: number) {
    ...
    if (isVertical ? contentHeight.current === newContentSize : contentWidth.current === newContentSize) {
      return;
    }
  ...
  const viewStyle = isVertical ? { height: dividerHeight } : { width: dividerWidth };
